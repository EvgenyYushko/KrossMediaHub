...
namespace AlinaKrossManager.BuisinessLogic.Services
{
	public class GoogleGenerativeLanguageModel : IGenerativeLanguageModel
	{
		...
		{
			var prompt = "Сделай транскрибацию этого аудио. Твоя задача — передать не только речь, но и эмоциональный фон. Любые звуковые артефакты, эмоции или изменения состояния (например: [вздох], [смех], [плач], [крик], [кашель], [пауза], [бормотание]) должны быть вписаны в текст в квадратных скобках именно в тот момент, когда они происходят. Не добавляй от себя никаких пояснений, верни только готовый текст.";
