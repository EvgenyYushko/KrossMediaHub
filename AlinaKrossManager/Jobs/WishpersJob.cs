		...
		public override async Task Execute(IJobExecutionContext context)
		{
			try
			{
				using (var scope = _serviceScopeFactory.CreateScope())
				{
					...
					var promt = "Придумай ОЧЕНЬ короткую, пошлую фразу (4-6 слов) для голосового сообщения на русском языке. " +
						"Это для приватного эротичного телеграм-канала модели Алины Кросс. " +
						"Фраза должна быть максимально возбуждающей, чтобы у участников сразу появилось лёгкое возбуждение. " +
						"Можно использовать развратные слова, стоны, хрипы и тяжёлое дыхание. " +
						"Добавляй интонации и вздохи в квадратных скобках [стонет], [дыхание], [шепчет], [громко стонет] и т.д. " +
						"Сделай максимально сексуально и эротично." +
					...
					var stream = await aiFacade.GenerateVoiceAsync(promtAi, sexySettings);
